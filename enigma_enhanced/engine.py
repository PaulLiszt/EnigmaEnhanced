    ...
    def load_key(key_data: dict) -> dict:
        ...
        if version in ["0.2.1", "0.2.2", "0.2.3", "0.2.4"]:
            ...
            if "directions" in key_data:
                key_data["rotor_directions"] = [d == 1 for d in key_data["directions"]]
            else:
        ...
        # 确保反射器是整数列表
        if "reflector" in key_data and not all(
            isinstance(x, int) for x in key_data["reflector"]
        ):
            key_data["reflector"] = list(range(256))
    ...
    def _derive_key(seed: bytes, salt: bytes, length: int) -> bytes:
        ...
        try:
            ...
            return EnigmaEngine._custom_hkdf(
                salt=salt, ikm=seed, info=b"enigma_key_material", length=length
            )

    @staticmethod
    def _step_rotors(
        positions: List[int], notches: List[int], directions: List[bool]
    ) -> List[int]:
        """完全符合历史恩尼格玛机的步进机制"""
    ...
    def encrypt(text: str, key: dict) -> str:
        ...
        for byte_val in text_bytes:
            result, positions = EnigmaEngine._process_byte(
                byte_val, positions, rotors, reflector, directions, notches, version
            )
...
def cli():
    ...
    parser.add_argument(
        "-v", "--version", action="version", version="EnigmaEnhanced 0.2.4"
    )
    parser.add_argument(
        "--genkey",
        nargs="?",
        const="enigma_key.json",
        metavar="KEYFILE",
    ...
    parser.add_argument(
        ...
        help="Key file to use (default: enigma_key.json)",
    )
    parser.add_argument("--encrypt", metavar="TEXT", help="Text to encrypt")
    parser.add_argument(
        "--decrypt",
        metavar="CIPHERTEXT",
        help='Ciphertext to decrypt (must start with "ENC:")',
    )
