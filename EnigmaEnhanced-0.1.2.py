...
class EnigmaEnhanced:
    ...
    def generate_key(self, filename="enigma_key.json"):
        ...
        self.key = key_data
        print(
            f"密钥已生成并保存到 {filename} The key has been generated and saved to {filename}"
        )
        return key_data
    ...
    def _process_byte(
        self, byte_val: int, rotors: List[List[int]], reflector: List[int]
    ) -> int:
...
def main():
    ...
    while True:
        ...
        # 命令处理 - 所有命令以斜杠 (/) 开头 Command processing: All commands start with slash
        if user_input.startswith("/"):
            # 移除斜杠并分割命令 Remove slash and parse command
